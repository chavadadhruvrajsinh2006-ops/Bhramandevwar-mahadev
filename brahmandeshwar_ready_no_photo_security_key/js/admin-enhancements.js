...
(function(){
  ...
  window.handleGalleryFileSelect=function(e){ pendingGalleryFile=e.target.files?.[0]||null; if(pendingGalleryFile) uploadPreview(pendingGalleryFile,'galleryUploadPreview','galleryUploadPrompt','galleryDropzone'); };
  ...
  window.saveDarshan=async function(){
    try{
      ...
      // Keep only the newest active Today's Darshan gallery item.
      const {data:oldDarshanGallery}=await window.supabaseClient.from('gallery').select('id,image_path').eq('category','darshan');
      if(oldDarshanGallery?.length){
        const oldPaths=oldDarshanGallery.map(x=>x.image_path).filter(Boolean);
        await window.supabaseClient.from('gallery').delete().eq('category','darshan');
        if(oldPaths.length) await window.supabaseClient.storage.from('temple-images').remove(oldPaths).catch(()=>{});
      }
  ...
  // Gallery photo upload: direct device upload, no extra security key prompt.
  window.addGallery=async function(){
    try{
      await currentAdmin();
      const title=document.getElementById('galleryTitle').value.trim();
      const categoryLabel=document.getElementById('galleryCategory').value;
      const description=document.getElementById('galleryDesc').value.trim();
      if(!title){ showToast('શીર્ષક જરૂરી છે.','error'); return; }
      if(!pendingGalleryFile){ showToast('કૃપા કરી ફોટો અપલોડ કરો.','error'); return; }
      const category=categoryLabel==='ગર્ભગૃહ'?'garbhagruh':'sthapatya';
      const up=await uploadImage(pendingGalleryFile,'gallery');
      const r=await window.supabaseClient.from('gallery').insert({title,description:description||title,category,image_url:up.url,image_path:up.path,published:true,sort_order:0}).select().single();
      if(r.error) throw r.error;
      const saved=r.data;
      DATA.gallery.unshift({id:saved.id,dbId:saved.id,image:saved.image_url,title:saved.title,description:saved.description||saved.title,category:saved.category,catLabel:categoryLabel});
      if(typeof clearGalleryForm==='function') clearGalleryForm();
      renderGallery(galleryFilter); updateAdminStats();
      showToast('ફોટો સીધો અપલોડ થઈને ગેલેરીમાં પ્રકાશિત થયો.','success');
    }catch(e){ console.error(e); showToast(e?.message==='LOGIN_REQUIRED'?'કૃપા કરી પહેલા એડમિન લોગિન કરો.':'ગેલેરી ફોટો અપલોડ કરવામાં ભૂલ થઈ.','error'); }
  };
